...
process_ukgov_eng_lookup <- function(sourcefile,h5filename, output_area_sf, grid_names) {
  ...
  oa_subdivisions$grid1km$Datazone_component_id=c(1:length(oa_subdivisions$grid1km$grid_id))
  oa_subdivisions$grid10km$Datazone_component_id=c(1:length(oa_subdivisions$grid10km$grid_id))
  #Make dataframes of the area of each datazone comoonent in each grid cell at both 1km and 10km
  intersection_area1km <- data.frame(grid1km_id = oa_subdivisions$grid1km$grid_id,
  ...
  intersection_area10km <- data.frame(grid10km_id = oa_subdivisions$grid10km$grid_id,
  ...
  grid_lookup=pivot_longer(grids.array,"1-1":"58-66", names_to="grid10km_id",values_to="grid1km_id")
