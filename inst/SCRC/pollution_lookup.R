...
doi_or_unique_name <- "UK national grid pollution data lookup"
...
source_filename <- list(shapefile = paste0(version_number, ".zip"),
...
# ../../srv/ftp/scrc/[product_name]
product_name <- "geography/national_grid"
...
original_source_name1 <- "Github/charlesroper/OSGB_Grids"
original_source_name2 <- "UK Air Information Resource"

original_source_name <- list(shapefile = original_source_name1,
                             "pollution/example" = original_source_name2)
...
original_sourceId <- list(shapefile = original_sourceId1,
                          "pollution/example" = original_sourceId2)
...
original_root = list(shapefile = "https://github.com/",
                     "pollution/example" = "https://uk-air.defra.gov.uk/datastore/pcm/")
original_path = list(shapefile = "charlesroper/OSGB_Grids/archive/master.zip",
                     "pollution/example" = "mappm252018g.csv")

for(i in seq_len(length(original_root))){
  if(names(original_root[i]) == "shapefile"){
    download_from_url(source_root = original_root[[i]],
                      source_path = original_path[[i]],
                      path = file.path("data-raw", product_name,
                                       names(original_root)[i]),
...
source_filename <- list(shapefile = file.path("OSGB_Grids-master", "shapefile", "OSGB_grid_1km.shp"),
                        "pollution/example" = paste0(version_number, ".csv"))
...
submission_script <- "process_pollution_lookup.R"
